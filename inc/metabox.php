...
<label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="claim-date-published">Claim Date Published:</label>
  <input class="w-full" type="date" style="height: 2.2em" id="claim-date-published" name="political-claim-date-published" value="' . ( $data['type'] === 'Factcheck' && isset( $data['itemReviewed']['datePublished'] ) ? $data['itemReviewed']['datePublished'] : '' ) . '">
</div>';
...
<div class="flex flex-row items-center gap-4">
	...
	$claimPublishedDate = '<div>
<label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="claim-date-published">Claim Date Published:</label>
  <input class="w-full" type="date" style="height: 2.2em" id="claim-date-published" name="debunk-claim-date-published" value="' . ( $data['type'] === 'Debunk' && isset( $data['itemReviewed']['datePublished'] ) ? $data['itemReviewed']['datePublished'] : '' ) . '">
</div>';
...
function ee24_publish_data( $post_id, $post ) {
	...
	if ( $publishedData['externalId'] ?? null ) {
		...
		try {
			...
			set_transient( "ee24_success", $data['externalId'], 25 );
		} catch ( \Throwable $e ) {
...
add_action( 'publish_post', 'ee24_publish_data', 12, 2 );
...
function updateEE24Metadata( $post_id ) {
	if ( array_key_exists( 'article-type', $_POST ) ) {
		...
		$data['itemReviewed'] = [
			'datePublished'  => $_POST['political-claim-date-published'] ?: $_POST['debunk-claim-date-published'],
			'author'         => $_POST['author'],
